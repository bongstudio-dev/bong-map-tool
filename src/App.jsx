...
const CENTROIDS = {
  "156": [105, 35], "356": [79, 22], "643": [100, 60], "840": [-97, 38],
  "364": [53, 32], "360": [120, -2], "586": [70, 30], "818": [30, 27],
  "634": [51.2, 25.3], "682": [45, 24], "124": [-106, 56], "076": [-52, -10],
  "032": [-64, -34], "792": [35, 39], "036": [134, -25], "764": [101, 15],
  "566": [8, 10], "012": [3, 28], "484": [-102, 23], "170": [-74, 4],
  "858": [-56, -33], "218": [-78, -2], "600": [-58, -23],
};
...
function shouldUseWhiteText(opacity) {
  const base = hexToRgb(BASE_COLOR_HEX), bg = hexToRgb(BG_COLOR);
...
function decodeTopojson(topo, objectName) {
  ...
  function decodeArc(arcIndex) {
    ...
    let x = 0, y = 0;
    const coords = arc.map(p => { x += p[0]; y += p[1]; return [x * transform.scale[0] + transform.translate[0], y * transform.scale[1] + transform.translate[1]]; });
    return arcIndex < 0 ? coords.reverse() : coords;
  }
  function decodeRing(ring) { let coords = []; ring.forEach(i => { coords = coords.concat(decodeArc(i)); }); return coords; }
  return {
    type: "FeatureCollection",
    features: obj.geometries.map(geom => {
      let coordinates;
      if (geom.type === "Polygon") coordinates = geom.arcs.map(decodeRing);
      else if (geom.type === "MultiPolygon") coordinates = geom.arcs.map(p => p.map(decodeRing));
      else coordinates = [];
...
function resolveOverlaps(positions, data, iterations = 30) {
  ...
  for (let iter = 0; iter < iterations; iter++) {
    ...
    for (let i = 0; i < items.length; i++) {
      for (let j = i + 1; j < items.length; j++) {
        ...
        const aL = a.x - a.w / 2 - BADGE_PAD, aR = a.x + a.w / 2 + BADGE_PAD;
        const aT = a.y - a.h - BADGE_PAD, aB = a.y + BADGE_PAD;
        const bL = b.x - b.w / 2 - BADGE_PAD, bR = b.x + b.w / 2 + BADGE_PAD;
        const bT = b.y - b.h - BADGE_PAD, bB = b.y + BADGE_PAD;
  ...
  const result = {};
  items.forEach(item => { result[item.country] = { x: Math.round(item.x), y: Math.round(item.y) }; });
  return result;
}
  ...
  return (
    <div style={{ marginBottom: 8 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 1 }}>
        ...
        <span style={{ fontSize: 9, fontWeight: 700, color: "#031A42" }}>{value}{unit || ""}</span>
      </div>
...
function useMapDrag(setMapOffset) {
  ...
  const onMouseDown = useCallback((e) => {
    ...
    setMapOffset(prev => { offsetRef.current = prev; return prev; });
  }, [setMapOffset]);
...
function BadgeDraggable({ country, icon, badgeRadius, badgeStroke, onDrag, posX, posY }) {
  ...
  useEffect(() => { if (textRef.current) setTextW(textRef.current.getBBox().width); }, [country]);
  ...
  const padX = 8, flagW = 24, flagH = 18, gap = 6;
  ...
  return (
    <g onMouseDown={handleMouseDown} style={{ cursor: dragging ? "grabbing" : "grab" }}>
      <rect x={badgeX} y={badgeY} width={totalW} height={badgeH}
        rx={badgeRadius} fill="white" stroke={BADGE_STROKE_COLOR} strokeWidth={badgeStroke} />
      <FlagIcon
        icon={icon}
        x={badgeX + padX}
        y={badgeY + (badgeH - flagH) / 2}
        w={flagW}
        h={flagH}
      />
      <text ref={textRef} x={badgeX + padX + flagW + gap} y={badgeY + badgeH / 2 + 1}
        fontSize={11} fontWeight={700} fill="#031A42"
        fontFamily="'DM Sans', sans-serif" dominantBaseline="central"
        style={{ pointerEvents: "none" }}>{country}</text>
    </g>
  );
}

export default function ChoroplethTool() {
  ...
  }, [data, geoData, getCountryCenter]);

  const maxVal = Math.max(...data.map(d => d.value), 1);
  const getOpacity = (val) => (val / maxVal) * 0.80 + 0.20;

  const valueMap = {};
  data.forEach(d => { const info = COUNTRY_DB[d.country]; if (info) valueMap[info.id] = d.value; });

  const updateData = (index, field, value) => {
    setData(prev => prev.map((d, i) => i === index ? { ...d, [field]: field === "value" ? Number(value) || 0 : value } : d));
  };
  const addRow = () => setData(prev => [...prev, { country: "", value: 0 }]);
  const removeRow = (index) => setData(prev => prev.filter((_, i) => i !== index));

  const handleLabelDrag = useCallback((country) => (x, y) => {
  ...
  const handleCopyPositions = () => {
    const lines = data
      ...
      .map(d => `  "${d.country}": { x: ${Math.round(labelPositions[d.country].x)}, y: ${Math.round(labelPositions[d.country].y)} },`);
  ...
  const exportSVG = () => {
    ...
    const a = document.createElement("a"); a.href = url; a.download = "choropleth_map.svg"; a.click();
  ...
  const exportPNG = () => {
    ...
    img.onload = () => {
      ...
      const a = document.createElement("a"); a.href = canvas.toDataURL("image/png"); a.download = "choropleth_map.png"; a.click();
    };
    img.src = "data:image/svg+xml;base64," + btoa(unescape(encodeURIComponent(svgStr)));
  };

  const inputStyle = {
    width: "100%", padding: "5px 7px", border: "1px solid #e5e5e5", borderRadius: 5,
    fontSize: 12, fontFamily: "'DM Sans', sans-serif", boxSizing: "border-box",
  };

  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif", display: "flex", height: "100vh", background: "#f8f7f4", color: "#031A42" }}>
      <link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;700;800&display=swap" rel="stylesheet" />

      {/* Left Panel */}
      <div style={{ width: 270, padding: "14px 12px", borderRight: "1px solid #e0ddd5", overflowY: "auto", background: "#fff", flexShrink: 0 }}>
        <div style={{ fontSize: 14, fontWeight: 800, marginBottom: 14, color: "#031A42", letterSpacing: -0.3 }}>Bong Map Tool</div>

        <div style={{ marginBottom: 10 }}>
          <div style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, color: "#bbb", marginBottom: 2 }}>Título</div>
          <input value={title} onChange={e => setTitle(e.target.value)} style={inputStyle} />
        </div>
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, color: "#bbb", marginBottom: 2 }}>Fuente</div>
          <input value={source} onChange={e => setSource(e.target.value)} style={inputStyle} />
        </div>

        <div style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, color: "#bbb", marginBottom: 6 }}>Controles visuales</div>
        <div style={{ background: "#fafaf8", borderRadius: 8, padding: "10px 10px 2px", marginBottom: 6, border: "1px solid #f0ede5" }}>
          ...
          <Slider label="Tamaño %" value={pctSize} onChange={setPctSize} min={6} max={28} step={1} unit="px" />
          ...
          <Slider label="Línea conector" value={connectorStroke} onChange={setConnectorStroke} min={0} max={2} step={0.1} unit="px" />
        </div>

        <button onClick={() => setMapOffset({ x: 0, y: 0 })} style={{
          width: "100%", padding: "5px", border: "1px solid #e0e0e0", borderRadius: 5,
          background: "#fafaf8", fontSize: 9, fontWeight: 600, color: "#666", cursor: "pointer",
          fontFamily: "'DM Sans', sans-serif", marginBottom: 4,
        }}>↻ Reset posición mapa</button>

        <div style={{ display: "flex", gap: 3, marginBottom: 14 }}>
          <button onClick={handleResolveOverlaps} style={{
            flex: 1, padding: "5px", border: "1px solid #e0e0e0", borderRadius: 5,
            background: "#fafaf8", fontSize: 9, fontWeight: 600, color: "#666", cursor: "pointer",
            fontFamily: "'DM Sans', sans-serif",
          }}>↕ Resolver overlap</button>
          <button onClick={handleCopyPositions} style={{
            flex: 1, padding: "5px", border: "1px solid #e0e0e0", borderRadius: 5,
            ...
            transition: "all 0.2s",
          }}>{copied ? "✓ Copiado" : "📋 Copiar posiciones"}</button>
        </div>

        <div style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, color: "#bbb", marginBottom: 6 }}>Datos</div>
        <div style={{ display: "flex", gap: 3, marginBottom: 3, padding: "0 2px" }}>
          <div style={{ flex: 1, fontSize: 8, fontWeight: 700, color: "#ccc" }}>PAÍS</div>
        ...
        {data.map((d, i) => {
          ...
          return (
            <div key={i} style={{ display: "flex", gap: 3, marginBottom: 2, alignItems: "center" }}>
              {info && (
                <img src={flagUrl(info.icon)} alt="" style={{ width: 18, height: 14, borderRadius: 2, objectFit: "cover", flexShrink: 0 }} />
              )}
              {!info && <div style={{ width: 18, height: 14, borderRadius: 2, background: "#eee", flexShrink: 0 }} />}
              <input value={d.country} onChange={e => updateData(i, "country", e.target.value)} placeholder="País"
                style={{ flex: 1, padding: "4px 5px", border: "1px solid #eee", borderRadius: 4, fontSize: 11, fontFamily: "'DM Sans', sans-serif", minWidth: 0 }} />
              <input type="number" value={d.value} onChange={e => updateData(i, "value", e.target.value)}
                style={{ width: 40, padding: "4px 4px", border: "1px solid #eee", borderRadius: 4, fontSize: 11, fontFamily: "'DM Sans', sans-serif", textAlign: "right" }} />
              <button onClick={() => removeRow(i)} style={{
                width: 18, height: 22, border: "none", background: "transparent", color: "#ccc", fontSize: 13, cursor: "pointer", padding: 0, lineHeight: 1
        ...
        <button onClick={addRow} style={{
          width: "100%", padding: "4px", border: "1px dashed #ddd", borderRadius: 4,
          background: "transparent", fontSize: 10, color: "#bbb", cursor: "pointer",
          fontFamily: "'DM Sans', sans-serif", marginTop: 3, marginBottom: 14
        }}>+ Agregar país</button>

        <div style={{ display: "flex", gap: 4 }}>
          <button onClick={exportSVG} style={{
            flex: 1, padding: "8px", border: "none", borderRadius: 6,
            background: "#4086FF", color: "white", fontSize: 11, fontWeight: 700,
            cursor: "pointer", fontFamily: "'DM Sans', sans-serif"
          }}>Exportar SVG</button>
          <button onClick={exportPNG} style={{
            flex: 1, padding: "8px", border: "none", borderRadius: 6,
            background: "#2D8035", color: "white", fontSize: 11, fontWeight: 700,
            cursor: "pointer", fontFamily: "'DM Sans', sans-serif"
          }}>Exportar PNG</button>
        </div>

        <div style={{ fontSize: 8, color: "#ccc", marginTop: 8, lineHeight: 1.5 }}>
          Banderas: Emoji One v1 (CC BY-SA 4.0). Arrastrá el mapa para panear, las etiquetas para reposicionar.
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", padding: 14 }}>
        ...
        {geoData && (
          <svg ref={svgRef} viewBox={`0 0 ${width} ${height}`}
            style={{ width: "100%", maxHeight: "100%", background: BG_COLOR, borderRadius: 6, cursor: mapDragging ? "grabbing" : "default" }}
            ...
            {/* LAYER 2: Connectors + percentages */}
                  ...
                  <g key={`conn-${d.country}-${i}`}>
                    {connectorStroke > 0 && (
                      <line x1={pos.x} y1={pos.y} x2={center.x} y2={center.y}
                        stroke="#7a8ba8" strokeWidth={connectorStroke} strokeDasharray="2,2" opacity={0.4} />
                    )}
                    <text x={center.x} y={center.y}
                      fontSize={pctSize} fontWeight={800} fill={pctColor}
                      fontFamily="'DM Sans', sans-serif" textAnchor="middle" dominantBaseline="central">
                      {d.value}%
                    </text>
                  </g>
                );
            ...
            <text x={40} y={36} fontSize={16} fontWeight={700} fill="#003DA5"
              fontFamily="'DM Sans', sans-serif" style={{ pointerEvents: "none" }}>{title}</text>

            {/* LAYER 4: Source bg */}
            <rect x={0} y={height - 30} width={width} height={30} fill={BG_COLOR} opacity={0.85} />
            <text x={width - 40} y={height - 14} fontSize={10} fill="#999"
              fontFamily="'DM Sans', sans-serif" textAnchor="end" style={{ pointerEvents: "none" }}>{source}</text>
            ...
            <g>
              {data.map((d, i) => {
                ...
                if (!info || !pos) return null;
                return (
                  <g key={`badge-${d.country}-${i}`} transform={`translate(${pos.x}, ${pos.y})`}
                    onMouseDown={(e) => e.stopPropagation()}>
                    <BadgeDraggable
                      country={d.country} icon={info.icon}
                      badgeRadius={badgeRadius} badgeStroke={badgeStroke}
